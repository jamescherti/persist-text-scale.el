...
(defcustom persist-text-scale-restore-once t
  "If non-nil, only restore text scale once per buffer.
Typically when the buffer is loaded."
  :type 'boolean
  :group 'persist-text-scale)

(defcustom persist-text-scale-verbose nil
  "If non-nil, display messages during text scale restoration."
  :type 'boolean
  :group 'persist-text-scale)

(defcustom persist-text-scale-buffer-type-function nil
  "Optional function to override the buffer type determination.
This overrides the function `persist-text-scale--buffer-type'. If non-nil, this
function will be called to determine the buffer type name. It should return a
string representing the buffer type name or nil to use the default behavior
based on `major-mode`."
  :type '(choice (const :tag "None" nil) function)
  :group 'persist-text-scale)

(defcustom persist-text-scale-autosave-interval (* 7 60)
  "The interval between autosaves of persist text scale data.
...
(defvar persist-text-scale--data nil
  "Alist to store text scale per major mode.")

(defvar persist-text-scale--previous-text-scale 1
  "Last text scale that the user has chosen.")
...
timer. This should not cause noticeable delays for users --
`persist-text-scale-autosave' executes in under 5 ms on my system."
  ...
  (let ((mode (persist-text-scale--buffer-type)))
    (when (and mode persist-text-scale--data)
      (let ((scale (or (cdr (assoc mode persist-text-scale--data))
                       persist-text-scale--previous-text-scale)))
        (when scale
          (let ((amount scale))
            amount))))))
  ...
  (when (bound-and-true-p persist-text-scale-mode)
    (let ((buffer-type (persist-text-scale--buffer-type)))
      ...
      (let ((cons-value (and persist-text-scale--data
        ...
        (setq persist-text-scale--previous-text-scale text-scale-mode-amount)))))
...
(defun persist-text-scale-load ()
  "Save data from `persist-text-scale-file'."
...
(define-minor-mode persist-text-scale-mode
  ...
  (if persist-text-scale-mode
      (progn
        ...
        ;; (add-hook 'window-configuration-change-hook #'persist-text-scale-restore)
        ;; (add-hook 'window-state-change-hook #'persist-text-scale-restore)
        )
    (persist-text-scale--cancel-timer)
